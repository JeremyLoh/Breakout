# breakout-6: Pretty colors update for level maker

Have the level maker create random levels by keeping track of alternative color bricks flag and skip bricks flag for each row

The bricks for each row are made to be odd, for symmetry in the row (left and right equal margins to edge of screen)

For each row, we also create 2 flags for keeping track of whether to skip or alternate the current brick. These flags need to be modified on each brick to maintain their order.

## Lua Continue statement (hack)
